...
delayIfNecessaryForDdl <- function(sql) {
  ...
  delayIfNecessary(sql, regexForDdl, ddlExecutionTimes, getOption("bigQueryConnectorDdlDelay", default = 5))
}

delayIfNecessaryForInsert <- function(sql) {
  regexForInsert <- "(^INSERT\\s+INTO)\\s+([a-zA-Z0-9_$#-]*\\.?\\s*(?:[a-zA-Z0-9_]+)*)"
  delayIfNecessary(sql, regexForInsert, insertExecutionTimes, getOption("bigQueryConnectorInsertDelay", default = 5))
}
